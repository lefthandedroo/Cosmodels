...
def vectorfield(v, t, w, lamb):
    ...
    """
    (a, a_dot, e_dash_m, e_dash_de, omegam, omegade) = v
    (w_m, w_de) = w
    ...
    f = [# a_dot
         ...
         (-a/2) * (e_dash_m * (1+3*w_m) + e_dash_de * (1+3*w_de)), 
         # e'_dotm
         -3 * (a_dot/a) * (e_dash_m * (1 + w_m -lamb/3 * a/a_dot 
              * e_dash_de/e_dash_m )),
         # e'_dotde
         -3 * (a_dot/a) * (e_dash_de * (1 + w_de +lamb/3 * a/a_dot)),
         # omegam_dot
         (H0**2) * e_dash_m * (2*a/a_dot) * (1 - a * (-a/2) 
             * (e_dash_m * (1+3*w_m) + e_dash_de * (1+3*w_de)) 
             / a_dot**2) + (a/a_dot)**2 * (-3 * a_dot/a * e_dash_m * (1+w_m)),
...
while True:
    ...
    xlabel('time in $H_0^{-1}$')
    grid(True)
    plot(t_cut, omegam, 'c', lw=1)
    plot(t_cut, omegade, 'k', lw=1)
    plot(t_cut, omegatot, 'tab:orange', ls= 'dashed', lw=1, 
         markevery=np.int(equiv))
